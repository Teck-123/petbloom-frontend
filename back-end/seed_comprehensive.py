...
Adds realistic pets and products with images, prices, and all required fields
"""
...
async def seed_pets():
    ...
    try:
        ...
        for pet_data in PETS_DATA:
            try:
                existing = await prisma_client.pet.find_first(
                    where={"name": pet_data["name"]}
                )
                ...
                await prisma_client.pet.create(data=pet_data)
                created_count += 1
                logger.info(f"✓ Created pet: {pet_data['name']}")
            except Exception as e:
                logger.error(f"Error creating pet '{pet_data['name']}': {str(e)}")
                continue
        
        logger.info(f"✓ Successfully created {created_count}/{len(PETS_DATA)} pets")
...
async def seed_products():
    ...
    try:
        ...
        for product_data in PRODUCTS_DATA:
            try:
                existing = await prisma_client.product.find_first(
                ...
                await prisma_client.product.create(data=product_data)
                created_count += 1
        ...
        logger.info(f"✓ Successfully created {created_count}/{len(PRODUCTS_DATA)} products")
        return created_count
    except Exception as e:
...
async def main():
    ...
    try:
        logger.info("=" * 80)
        logger.info("Starting PetBloom Database Seeding")
        logger.info("=" * 80)
        
        await prisma_client.connect()
        logger.info("✓ Connected to database")
        
        pets_count = await seed_pets()
        products_count = await seed_products()
        
        logger.info("=" * 80)
        logger.info("SEEDING COMPLETE!")
        logger.info(f"✓ Created {pets_count} pets")
        logger.info(f"✓ Created {products_count} products")
